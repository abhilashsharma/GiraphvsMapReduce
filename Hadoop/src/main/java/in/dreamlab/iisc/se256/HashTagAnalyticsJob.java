 ...
 *
 */
public class HashTagAnalyticsJob extends Configured implements Tool {
	private static final Logger LOG = Logger.getLogger(HashTagAnalyticsJob.class);
 	
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(), new HashTagAnalyticsJob(), args);
        System.exit(res);
	}
...
//		conf.setStrings("cntr_arr", arr);
	    ...
	    Job job = Job.getInstance(conf, "HashTagAnalytics");
	    ...
	    job.setJarByClass(HashTagAnalyticsJob.class);	    
	    job.setMapperClass(HashTagAnalyticsMapper.class);
//	    job.setCombinerClass(LongSumReducer.class);
	    job.setReducerClass(HashTagAnalyticsReducer.class);
...
//		job.setInputFormatClass(WARCFileInputFormat.class);
        ...
        int jobvalue=job.waitForCompletion(true) ? 0 : 1;
        
        conf.set("HashMapPath", outputPath);
        
        Job job2 = Job.getInstance(conf, "HashTagAnalytics2");
	    ...
	    /////////////////////////////////////////////////////////
	    // Set classes for Job, Mapper, Combiner and Reducer. 
	    job2.setJarByClass(HashTagAnalyticsJob.class);	    
	    job2.setMapperClass(HashTagAnalyticsMapper2.class);
//	    job.setCombinerClass(LongSumReducer.class);
	    job2.setReducerClass(HashTagAnalyticsReducer2.class);
		...
		if (fs1.exists(new Path(outputPath+"Closed"))) { // delete output folder if already present
			fs1.delete(new Path(outputPath + "Closed"), true);
		}
		FileOutputFormat.setOutputPath(job2, new Path(outputPath + "Closed"));

		LOG.info("Input path: " + outputPath); 
		LOG.info("Output path: " + outputPath + "Closed");
